...
const aromaFilters = {
  tinto: {
    ...
    "frutos maduros": ["%frutos maduros%", "%cereza%", "%mora%", "%baya%", "%ciruela%", "%grosella%"],
  },
  blanco: {
...
const maridajeFilters = {
  ...
  blanco: {
    ...
    carnes: ["%pollo%", "%pavo%", "%cerdo%", "%carne"],
  },
  espumoso: {
    "pescados y mariscos": ["%pescados%", "%mariscos%", "%caviar%"],
    aperitivos: ["%quesos%", "%entrantes%", "%postres%", "%ensaladas%"],
    carnes: ["%pollo%", "%pavo%", "%jamón%", "%carne%", "%carnes%"],
  },
};
...
export const filterWines = async (req, res) => {
  try {
    ...
    // ✅ Цвет и тип — теперь работают независимо
    if (color === "tinto") {
      whereConditions.color = "Tinto";
      whereConditions.type = { [Op.ne]: "Espumoso" }; // тино не может быть игристым
    } else if (color === "blanco") {
      whereConditions.color = "Blanco";
      if (type !== "espumoso") {
        whereConditions.type = { [Op.ne]: "Espumoso" }; // исключаем игристые, если явно не выбрали их
      }
    }

    if (type === "espumoso") {
      whereConditions.type = "Espumoso"; // работает и при пустом color
    }

    // ✅ maridaje — теперь берём фильтры даже при пустом color
    if (maridaje) {
      const maridajeConditions = [];
      const filtersForMaridaje = maridajeFilters[color || type]?.[maridaje];
    ...
    // ✅ aroma — тоже используем color || type
    if (aroma) {
      const aromaConditions = [];
      const aromaList = aroma.split(",").map((a) => a.trim());
      aromaList.forEach((a) => {
        const filtersForAroma = aromaFilters[color || type]?.[a];
        if (filtersForAroma) {
    ...
    if (price) {
      ...
      if (!isNaN(min) && !isNaN(max)) {
        ...
        whereConditions.price = { [Op.gte]: min };
      }
    }
    ...
    console.log(`🍷 Найдено вин: ${wines.length}`);
    ...
    return res.status(404).json({ message: "No wines found with the given filters" });
